...
type JournalEntriesCalenderProps = {
  entries: JournalEntry[]
}
export function JournalEntriesCalendar({ entries }: JournalEntriesCalenderProps) {
  const [year, setYear] = useState<number>(DateFactory.getTodayDate().getFullYear());
  const [currentMonth, setCurrentMonth] = useState<number>(DateFactory.getTodayDate().getMonth());
  ...
  const getEntry = (year: number, month: number, day: number) => {
  ...
  const generateMonthGrid = (month: number) => {
  ...
  const MonthGrid = ({ month, monthIndex }: {month: string, monthIndex: number}) => (
    <div
      key={month}
