...
import { deleteEntryImage } from '@/app/actions/journalEntryActions';
import { MediaItemUrl } from '@/app/models/media-item-string';

type ImageGridProps = {
  images: MediaItemUrl[],
  setImages: React.Dispatch<React.SetStateAction<MediaItemUrl[]>>
}

export default function ImageGrid({ images, setImages }: ImageGridProps) {
  const [selectedImage, setSelectedImage] = useState<MediaItemUrl | undefined>(undefined);
  const [imageToDelete, setImageToDelete] = useState<MediaItemUrl | undefined>(undefined);
  ...
  const handleDelete = async () => {
    if (imageToDelete) {
      ...
      await deleteEntryImage(imageToDelete.mediaKey);
